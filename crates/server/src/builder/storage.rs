...
impl StorageMetadataBuilder {
    ...
    pub fn from_env() -> Self {
        Self::new()
            .storage_path(std::env::var("PSM_STORAGE_PATH")
                .unwrap_or_else(|_| "/var/psm/storage".to_string())
                .into())
            .metadata_path(std::env::var("PSM_METADATA_PATH")
    ...
    pub async fn build(
        self,
    ) -> Result<(Arc<dyn StorageBackend>, Arc<dyn MetadataStore>), String> {
        #[cfg(feature = "postgres")]
        {
            ...
            let metadata = crate::metadata::postgres::PostgresMetadataStore::new(&database_url).await?;

            return Ok((Arc::new(storage), Arc::new(metadata)));
        }

...
/// one asks the question none of those can: whether an account created by an
/// earlier build still works under this one. Deployed accounts are immutable and
/// their procedure roots fix at creation, so a contract pin bump strands them,
/// and nothing else in the suite would notice.
///
/// A failure here is therefore not automatically a regression. If the pin moved,
/// the correct reading is that accounts created before it cannot be driven by
/// this build, which is a release note rather than a bug, and the account has to
/// be recreated the way the treasury does.
///
/// Environment-blocked without a key, so a run without the secret is not a
/// failure: this is the one account the suite cannot conjure for itself.
pub async fn open_heritage(runner: &Runner, scheme: Scheme, run_tag: &str) -> ActionOutcome {
    let Some(context) = runner.live.as_ref() else {
        return ActionOutcome::failed_setup("the live context is not configured");
    };
    let network = context.network.as_str();
    let key = std::env::var(format!("QUAL_HERITAGE_KEY_{network}"))
        .ok()
        .filter(|value| !value.trim().is_empty());
    let id = std::env::var(format!("QUAL_HERITAGE_ACCOUNT_{network}"))
        .ok()
        .filter(|value| !value.trim().is_empty());
    let (Some(key), Some(id)) = (key, id) else {
        return ActionOutcome::EnvironmentBlocked {
            reason: format!(
                "QUAL_HERITAGE_KEY_{network} and QUAL_HERITAGE_ACCOUNT_{network} are unset, so \
                 there is no long-lived account to drive; create one with `heritage-new`"
            ),
        };
    };

    let account_id = match AccountId::from_hex(id.trim()) {
        Ok(account_id) => account_id,
        Err(error) => {
            return ActionOutcome::failed_setup(format!(
                "QUAL_HERITAGE_ACCOUNT_{network} is not an account id: {error}"
            ));
        }
    };

    // 1-of-1 by construction: one persisted key is the whole point, and a larger
    // set would mean persisting every cosigner's key to prove the same thing.
    // The scheme comes from the manifest, and it matters more than usual here:
    // the signature scheme is baked into the account's on-chain code at
    // creation, so a heritage account can only ever speak the one it was born
    // with.
    let signer = match RunSigner::from_hex(scheme, &key) {
        Ok(signer) => signer,
        Err(error) => {
            return ActionOutcome::failed_setup(format!(
                "QUAL_HERITAGE_KEY_{network} is unusable as a {scheme:?} key: {error}"
            ));
        }
    };
    let signers = RunSigners {
        signers: vec![signer],
        threshold: 1,
    };
    ...
    // Pulled from GUARDIAN rather than rebuilt locally: the stored state is what
    // an earlier build wrote, and rebuilding it here would quietly reconstruct
    // it under today's contract, which is the very thing under test.
    if let Err(error) = clients[0].pull_account(account_id).await {
        return ActionOutcome::failed_product(format!(
            "the long-lived account {account_id} could not be loaded from GUARDIAN. If the \
             contract pin moved since it was created, accounts created before the move cannot \
             be driven by this build and it needs recreating: {error}"
        ));
    }

...
| Status | Meaning |
|---|---|
| `candidate` | Guardian accepted and signed it, but the matching Miden update has not yet been observed. An account may queue several candidates as a strictly ordered chain (each building on the previous one's post-state, up to `GUARDIAN_MAX_PENDING_CANDIDATES_PER_ACCOUNT`, default 4), so a client need not wait for canonicalization before pushing its next transaction. |
...
| Transaction died after approval (stranded candidate) | While the candidate waits out the grace + retry window, deltas and proposals that would exceed the account's candidate queue (default depth 4, `GUARDIAN_MAX_PENDING_CANDIDATES_PER_ACCOUNT`), or that build on the state the stranded candidate already claimed, are answered `409 conflict_pending_delta`; candidates queued behind it are parked as `retained` (`orphaned`) once it is abandoned | Call `POST /delta/candidate/abandon` (SDKs: `abandonCandidate` / `abandon_candidate`). The worker confirms over a short quarantine that the transaction did not land, flips the delta to `discarded` with reason `client_abandoned`, and releases the account — typically well under a minute. Poll via `abandonStatus` / `abandon_status`. |

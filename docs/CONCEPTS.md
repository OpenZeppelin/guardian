...
| Guardian database corruption (or restore from an older backup) | Stored state lags the chain: state verification fails for accounts whose on-chain commitment advanced past the stored state | Clients reject the unverifiable data. A device holding the newer state re-syncs it, or rotate to another operator — the guardian cannot regenerate lost deltas because guarded accounts are private. |

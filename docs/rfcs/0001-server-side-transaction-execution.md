...
The finite-expiration requirement is what makes the last outcome guaranteed rather than best-effort: even if the send never starts or the node silently drops the transaction, the reconciliation watch terminates within the expiration horizon. This is also why boundary-crossed executions need no re-submission machinery: Guardian knows exactly what it prepared and when it stops being submittable.
...
| `GUARDIAN_EXECUTION_EXPIRATION_HORIZON_BLOCKS` | No | TBD | Maximum allowed distance from the reference block to expiration; exceeding it prevents crossing the no-retry boundary. This bound guarantees reconciliation terminates. |

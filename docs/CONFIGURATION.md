...
| `GUARDIAN_MAX_REPLICAS` | `1` (code default); deployment **surge capacity** (`max(desired, autoscaling max) × deployment_maximum_percent`, 2 × capacity by default) set by the prod Terraform profile | Per-replica rate-limit divisor: each replica enforces `global / GUARDIAN_MAX_REPLICAS` so the fleet aggregate stays at or below the global limit **even during a rolling deploy**, when ECS can briefly run up to `deployment_maximum_percent` of capacity. Use the worst-case concurrent task count, not the running count. Drives rate-limiting only — coordination mode is backend-derived. Running below that capacity over-throttles (accepted; raise the global limits if steady-state throughput matters); an override is clamped up to the surge capacity by Terraform. Must be a positive integer when set: an invalid value **fails startup in prod** and is treated as `1` (warning, no partitioning) elsewhere. Also partitions the dashboard per-commitment challenge limits (clamped to ≥1 per replica so login is never fully denied). See [`runbooks/horizontal-scaling.md`](./runbooks/horizontal-scaling.md). |

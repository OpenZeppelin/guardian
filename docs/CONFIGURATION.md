...
| `GUARDIAN_RELEASE_SWEEP_ENABLED` | `true` | any | Runs the chain-driven release sweep (issue #434), a background task that walks every unreleased Miden account against the chain and releases those whose guardian switch never reached this server (offline switches, failed pushes, a network-dead old operator). A switch is recognised either through a pending `switch_guardian` proposal whose post-state the chain reached (works for private accounts) or, for public accounts, through the guardian key in the account's published on-chain storage. Set to `false` to leave release detection to the push path alone. |
| `GUARDIAN_RELEASE_SWEEP_ROTATION_SECONDS` | `21600` (6 h) | any | Target time for one full walk of the fleet: every eligible account is probed once per rotation, with the probes spread evenly over the window. |
| `GUARDIAN_RELEASE_SWEEP_MAX_RATE_PER_SECOND` | `5` | any | Upper bound on accounts the walk probes per second (one cheap `GetAccount` each), i.e. the sweep's share of chain-node RPC capacity. A fleet too large to cover in one rotation at this rate simply takes longer. |
| `GUARDIAN_RELEASE_SWEEP_PAGE_SIZE` | `100` | any | Accounts fetched from the metadata store per listing page during the walk. A batching detail only: the walk's cursor advances per visited account, never per page. |
| `GUARDIAN_RELEASE_SWEEP_HOT_INTERVAL_SECONDS` | `60` | any | Cadence of the hot pass, which re-probes only the accounts awaiting confirmation and the accounts with a pending `switch_guardian` proposal, so those release within a minute or two instead of a rotation. |
| `GUARDIAN_RELEASE_SWEEP_CONFIRMATIONS` | `2` | any | Consecutive observations of a foreign guardian key in published storage required before releasing on that evidence (a shield against one stale RPC read). A pending proposal whose post-state matches the chain exactly is proof and releases at once. |

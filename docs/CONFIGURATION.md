...
| Variable | Default | Notes |
|---|---|---|
| `GUARDIAN_RATE_LIMIT_ENABLED` | `true` | Master kill-switch for rate limiting on **both** transports (HTTP and gRPC; there is no per-transport toggle). Set `false` only in test environments. Client identity for keying comes from the ingress (rightmost `X-Forwarded-For` entry, then `X-Real-IP`, then the socket peer); deployments not behind the reference ALB must forward the client address on both listeners and restrict direct access to the server ports; see [PRODUCTION.md](./PRODUCTION.md#running-behind-your-own-ingress-non-aws). |
| `GUARDIAN_RATE_BURST_PER_SEC` | `10` (code default); `200` set by the prod Terraform profile | Token-bucket burst, per IP and endpoint path. Covers HTTP endpoints and gRPC methods from one shared store. Deployments sized for HTTP-only traffic should re-check these values: gRPC traffic (the Rust SDK's default transport) counts against the same budget since the transport-bypass fix. |
| `GUARDIAN_RATE_PER_MIN` | `60` (code default); `5000` set by the prod Terraform profile | Sustained rate, per IP across both transports. Same re-tuning note as the burst limit. |

...
| `GUARDIAN_CANONICALIZATION_ABANDON_QUARANTINE_SECONDS` | `15` | any | Minimum wall time between a client abandon request and discarding the candidate, so a transaction that lands late still canonicalizes instead of being thrown away. The account stays blocked to new deltas for this window. Lowering it shortens that block and increases the risk of discarding a late-landing transaction; `0` leaves the consecutive-check requirement as the only gate. |
| `GUARDIAN_CANONICALIZATION_ABANDON_QUARANTINE_CHECKS` | `2` | any | Consecutive at-base observations required before an abandon is finalized, guarding against a stale read. Must be greater than zero. |

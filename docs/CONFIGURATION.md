...
| `GUARDIAN_DASHBOARD_STATS_REFRESH_INTERVAL_SECS` | `60` | Cadence of the per-replica background refresh that maintains the `GET /dashboard/stats` aggregate (account counts, Miden asset totals, coverage) and the `accounts_by_auth_method` field of `GET /dashboard/info`. A request never triggers a refresh; responses carry the snapshot time as `as_of`, so the worst-case staleness is this interval plus one refresh's duration. Must be a positive integer. See [DASHBOARD.md](./DASHBOARD.md#aggregate-stats). |

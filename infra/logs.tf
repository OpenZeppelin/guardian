...
# Prod keeps a year of cluster and server logs; the EMF metrics group stays
# on log_retention_days because CloudWatch materializes those events as
# metrics on ingestion.
resource "aws_cloudwatch_log_group" "cluster" {
  name              = local.cluster_log_group_name
  retention_in_days = local.is_prod ? 365 : var.log_retention_days
}

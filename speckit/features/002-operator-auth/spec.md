...
Layers affected: a new authentication surface in the existing Guardian server (challenge/verify/logout endpoints plus session middleware) and a configuration channel for the operator allowlist. V1 is Falcon-only and assumes challenge signing happens in-browser through Miden Wallet. No changes are required to the multisig protocol, proposal lifecycle, or existing gRPC/HTTP client SDKs.
...
- Operator revocation by removing the entry from configuration and restarting (or hot-reloading) the dashboard backend.
...
- Operator self-service key rotation workflows beyond editing configuration and restarting.
- Detailed Miden Wallet UX and browser-integration design beyond the requirement that the dashboard request an in-browser Falcon signature from the wallet.
...
A registered Guardian operator visits the dashboard in their browser. They are prompted to approve a challenge signature in Miden Wallet using their registered Falcon key. After providing a valid signature, they receive a session cookie that transparently authenticates all subsequent dashboard requests until it expires or they log out.
...
An operator explicitly logs out at the end of their session, or an administrator removes an operator from the configuration and restarts the service. In either case, all existing sessions for that operator must stop working immediately.
...
**Independent Test**: After establishing a session, call the logout endpoint and confirm the session cookie stops being accepted. Separately, remove an operator entry from configuration, reload/restart the backend, and confirm any previously issued sessions for that operator are rejected.
...
2. **Given** an operator entry is removed from configuration and the backend reloads its allowlist, **When** a previously issued session cookie for that operator is presented, **Then** the request is rejected with 401 regardless of remaining session lifetime.
...
- **SC-005 — Operational revocation is fast**: Removing an operator from configuration and reloading the backend causes any session for that operator to be rejected on its next request, without a restart being required when hot-reload is enabled.
...
- **SC-007 — Low-friction operator onboarding**: Adding a new operator requires only appending one configuration entry and a reload/restart; no database migration, no code change, no per-operator secrets provisioned by the service.
...
- Operators already possess the Falcon private keys registered in configuration and access the dashboard from a browser environment where Miden Wallet can sign the challenge payload.
...
- Q: How do operators sign the challenge payload? → A: In-browser through Miden Wallet.

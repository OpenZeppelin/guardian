...
- [ ] T010 Implement the reservation methods on `FilesystemService` in `crates/server/src/storage/filesystem.rs` as read-modify-write under **`delta_write_lock`** (`filesystem.rs:25`) — not a new mutex, since admission must be atomic with respect to the candidate writes that lock serializes. Persist holder, fence, and expiry in the reservation file; create/renew/claim update them under the lock, and every execution-owned mutation validates the supplied holder/fence against that active reservation before writing. A stale or unfenced execution call returns `StaleLease`. Do not change the intentionally unfenced client `request_candidate_abandon` annotation or the `AlwaysLeader` canonicalization semantics
...
- [ ] T032a [US1] Validate `chain_anchor` at proposal admission for a Guardian-executable proposal (FR-056): refuse a missing or undeserializable anchor, an anchor whose block commitment differs from `tx_summary.block_commitment()`, any request that consumes a note not present in its `explicit_input_notes` (built-in consume-notes and custom producers alike; without pinning the tracked-block check below has nothing to check), and an anchor that does not track the creation block of every authenticated note so pinned. Admission lives in `crates/server/src/services/push_delta_proposal.rs` (`push_delta_proposal`, line 44), which today stores `chain_anchor` without reading it, so this is new behavior with its own error codes and tests on both backends
- [ ] T033 [P] [US1] Add integration test `crates/server/src/services/execute_proposal/tests.rs`: threshold-met Guardian-executable proposal → execute → poll to `committed`, asserting the delta canonicalized through the existing lifecycle and that **no new delta status value** appears (US1 scenarios 1–3)
- [ ] T034 [P] [US1] Add test in `crates/server/src/services/execute_proposal/tests.rs` asserting the reported state progression is one of the permitted transitions only (`pending → proving → submitted → committed`) and never absent or ambiguous (US1 scenario 2, FR-024)
...
- [ ] T039 [US1] Implement FR-045 steps 1–2 in `crates/server/src/services/execute_proposal/mod.rs`: select the valid signature subset and confirm the effective per-procedure threshold (FR-005, FR-006), then reproduce the transaction at the proposal's `ChainAnchor` and verify the binding to the signed summary (FR-007, FR-056, FR-057): deserialize the anchor (`ChainAnchor::read_from` already enforces header/chain consistency), refuse when its block commitment differs from `tx_summary.block_commitment()`, read the header at the anchored height from the node and refuse on commitment mismatch, take the reference header and `PartialBlockchain` from the anchor, refuse an authenticated input note whose creation block the anchor does not track, then make the fee conversion decision **before** the unsigned reproduction: leave a request with an auth arg untouched, otherwise commit chain-native 1/1 info from the anchored header under the declared salt (mirror `miden-client-0.16.0/src/transaction/mod.rs:1528-1560`; the outer helper is private, `commit_native_fee_conversion_info` is `pub(crate)`). Step 2 **must** precede step 3 — never acknowledge a transaction that does not reproduce the signed summary
- [ ] T040 [US1] Implement FR-045 steps 3–6 in `crates/server/src/services/execute_proposal/mod.rs`: issue the acknowledgment via the internal path, inject signature and acknowledgment advice (the fee conversion advice is already attached from T039), execute the authorized transaction through `network/miden/execution/`, re-verify the binding on the executed result, then read the chain height from the node and refuse with a distinct pre-boundary error when `ExecutedTransaction::expiration_block_num()` is at or below it (FR-058; mirror `miden-client-0.16.0/src/transaction/mod.rs:393-402`)
- [ ] T041 [US1] Implement FR-045 step 7 in `crates/server/src/services/execute_proposal/mod.rs`: prove via `RemoteTransactionProver` with the configured explicit timeout (FR-019, FR-020). On error, walk the `std::error::Error::source` chain into the log — the outermost `Display` is only "failed to prove transaction" and discards the gRPC status. Classify each failure transient-versus-permanent and retry transient failures with capped backoff under the held, renewed reservation (FR-055): the transient class MUST match transport-level failures (connection errors, i/o timeouts, deadline-exceeded) from the walked source chain, not just structured prover errors; stop retrying and fail-and-release when the failure is permanent, when the **executed** transaction's expiration block can no longer be met within the FR-046 horizon, or when that expiration block is at or below the chain height read at the retry (FR-058, `GUARDIAN_EXECUTION_ANCHOR_EXPIRED`); the horizon is measured from the reference block and does not advance with the tip, so the height check is the one that catches a stale anchor. No `ProvenTransaction` exists during a failed proving attempt; after proof succeeds, T042 reads `ProvenTransaction::expiration_block_num()` as the authoritative boundary/evidence value. Add stub-prover tests injecting both failure families (SC-040)
...
- [ ] T050 [US1] Wire the `committed` outcome through the extended `promote_candidate` in `crates/server/src/jobs/canonicalization/processor.rs`, using the storage primitive from T018. Promotion is what makes `committed` true, so it persists the outcome and releases the reservation inside its own transaction; `execution_reconcile` MUST NOT also write `committed` on observing `canonical` — two writers for one fact reintroduces the `remove_candidate` race FR-041 exists to prevent. Reconciliation covers only superseded and expired, which are US4 (FR-041, FR-053, SC-025)
...
- [ ] T059 [P] [US2] Add codec-rejection tests to `crates/server/src/services/execute_proposal/binding_tests.rs`: a bad checksum is rejected before deserialization; an incompatible `protocol_line`, unsupported `format_version`, and same-line unallowlisted `serializer_id` are each refused before deserialization with the contract's stable codec/protocol code (FR-014, FR-015, SC-029)
...
- [ ] T067a [US2] Finalize FR-016 proposer-count configuration and account-wide allocation so one proposer cannot consume another signer's allocated count capacity; two per proposer is the proposed default. Document the chosen configuration and creation-error contract before implementation
- [ ] T067b [US2] Implement atomic per-account/per-authenticated-proposer viable-count checks with insertion in both storage backends. Test concurrent creates, stale proposals freeing count capacity, and a capped proposer not preventing another signer from using its allocation. Propagate any creation error/config changes through both base clients, SDKs and their docs
- [ ] T067c [P] [US2] Verify creation and signature collection never execute a request or reserve an account; a proposal ready at creation still needs an explicit execution request. Verify Guardian acknowledgment cannot substitute for a missing cosigner signature. Exercise both SDKs and Falcon/ECDSA paths
...
- [ ] T079 [P] [US4] Add to `crates/server/src/services/execute_proposal/fault_injection_tests.rs`: a submission timeout reports `submitted`, refuses retry, retains the reservation, and resolves only after chain observation. Then make chain observation unavailable across the recorded expiration height: the execution remains `submitted`, the reservation remains held, outage health/metrics fire, and resolution resumes only after trustworthy observation recovers (US4 scenario 5, FR-030, FR-040)
...
- [ ] T089 [US4] Implement the **expired** evidence path in `crates/server/src/jobs/execution_reconcile/`: the chain passed `expiration_block` with the account still at `base_commitment` → `failed` / `GUARDIAN_EXECUTION_EXPIRED` (FR-040). On unavailable observation, retain the reservation and retry with capped backoff; expose health, metrics, and logs so operators restore or fail over RPC. Never settle or release on wall-clock time alone. Expiration supplies the finite chain-height bound once trustworthy observation is available
...
- [ ] T101a [US3] Pin every consumed note in the request for Guardian-executable consume-notes proposals in `crates/miden-multisig-client/src/transaction/consume.rs` (FR-056): build the request with `TransactionRequestBuilder::explicit_input_notes` carrying each authenticated `InputNote` (proof included) instead of relying on the executing client's store classification (`consume.rs:108-120` today). Keep the summary identical to the store-classified path for the same notes and anchor, and add a test that a fresh client with an empty store reproduces the proposer's summary from the request bytes plus the anchor alone
...
- [ ] T102a [P] [US3] Mirror T101a in `packages/miden-multisig-client/` with the WASM builder's explicit-input-notes equivalent, with the same empty-store reproduction test in the browser harness
- [ ] T103 [P] [US3] Add cross-SDK parity tests: both SDKs emit the same `serializer_id`, use the same 256-block built-in default, derive the same summary/proposal identity for the same effects, salt, and anchor (and a **different** identity from self-executed mode, because the expiration delta is signed on 0.16), and accept/reject finite/non-finite custom requests consistently (FR-012, FR-014, FR-051)
...
- [ ] T106 [US3] Add **every** new error code to the TS client error-code vocabulary in `packages/guardian-client/`, including `GUARDIAN_EXECUTION_ANCHOR_EXPIRED` (FR-058) and the FR-056 admission codes from T032a. This is exactly the gap that produced #353, where `candidate_landed` was missing
- [ ] T107 [P] [US3] Commit cross-language envelope fixtures under `fixtures/miden-multisig-client/`: identical `format_version`, `protocol_line`, full `serializer_id` (including prerelease), and `checksum` produced by both SDKs for the same transaction; a different-protocol-line fixture; a same-line/unallowlisted-serializer fixture; and an unsupported-format fixture. Every incompatibility MUST be refused before deserialization and wired into both SDK and server codec tests (SC-029, SC-010)
...
- [ ] T124 [P] Add execution metrics in `crates/server/src/metrics/`: executions by terminal state, proving duration, reservation age, reconciliation outcomes by evidence path, and chain-observation outage duration/failures. Internal phases and degraded reconciliation are diagnosable from metrics, health, and logs, never from the wire (FR-025, FR-040)
...
any service code. Then run Phase 3 to a working `committed`, and immediately follow

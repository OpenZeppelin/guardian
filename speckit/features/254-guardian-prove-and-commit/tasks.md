...
- [ ] T039 [US1] Implement FR-045 steps 1–2 in `crates/server/src/services/execute_proposal/mod.rs`: select the valid signature subset and confirm the effective per-procedure threshold (FR-005, FR-006), then reproduce the transaction at the proposal's `ChainAnchor` and verify the binding to the signed summary (FR-007, FR-056): deserialize the anchor (`ChainAnchor::read_from` already enforces header/chain consistency), refuse when its block commitment differs from `tx_summary.block_commitment()`, read the header at the anchored height from the node and refuse on commitment mismatch, take the reference header and `PartialBlockchain` from the anchor, and refuse an authenticated input note whose creation block the anchor does not track. Step 2 **must** precede step 3 — never acknowledge a transaction that does not reproduce the signed summary
- [ ] T040 [US1] Implement FR-045 steps 3–6 in `crates/server/src/services/execute_proposal/mod.rs`: issue the acknowledgment via the internal path, inject signature and acknowledgment advice plus the fee conversion advice derived from the anchored header when the request declares a `fee_conversion_salt` (FR-057; mirror `miden-client-0.16.0/src/transaction/mod.rs:1528-1550`, which is `pub(crate)`), execute the authorized transaction through `network/miden/execution/`, and re-verify the binding on the executed result

...
- [ ] T032a [US1] Validate `chain_anchor` at proposal admission for a Guardian-executable proposal (FR-056): refuse a missing or undeserializable anchor, an anchor whose block commitment differs from `tx_summary.block_commitment()`, and an anchor that does not track the creation block of every authenticated input note in the stored request; also refuse a consume-notes request that does not pin its notes through `explicit_input_notes`. Today the server stores the field without reading it (`crates/server/src/delta_summary/mod.rs:128`), so this is new behavior with its own error codes and tests on both backends

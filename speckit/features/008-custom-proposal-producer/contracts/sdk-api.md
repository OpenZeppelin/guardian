...
## Shared concepts
- **Artifact**: serialized `TransactionRequest` bytes (Rust `&[u8]`, TS `Uint8Array`). Not stored by the SDK or server (FR-015); the integration owns its recipe and supplies it at create and execute-prep.
...
- **Binding**: `commitment(summary(deserialize(artifact))) == proposal.id`, enforced in `prepare_custom_execution` before any ack/submit (FR-007/FR-020/FR-023).
...
/// `artifact`: serialized TransactionRequest bytes; `proposal_type`: free-form,
/// non-empty, not a modeled label.
pub async fn propose_custom(&mut self, artifact: &[u8], proposal_type: &str) -> Result<Proposal>;

/// For a threshold-met custom proposal: binding-check the artifact, fetch the
...
Integration execute flow (Rust): `let advice = client.prepare_custom_execution(id, &artifact).await?; let mut req = deserialize_transaction_request(&artifact)?; req.advice_map_mut().extend(advice); client.submit_transaction(req).await?;`

## TypeScript — `Multisig` (`packages/miden-multisig-client/src/multisig.ts`)

```ts
proposeCustom(artifact: Uint8Array, proposalType: string, nonce?: number): Promise<Proposal>;

// Binding-check, fetch ack, return advice. Does NOT submit.
prepareCustomExecution(proposalId: string, artifact: Uint8Array): Promise<AdviceMap>;
...
Integration execute flow (TS): `const advice = await multisig.prepareCustomExecution(id, artifactBytes); const finalReq = myBuilder.extendAdviceMap(advice).build(); await multisig.submitTransaction(finalReq);`

## Behavioral contract (both SDKs)
- `propose_custom` MUST: normalize the label (trim + lowercase) and reject it if empty, not `[a-z0-9_]+`, or a modeled label (FR-001/FR-021); deserialize artifact (else decode error, FR-016); derive the summary via `execute_for_summary`; package with the label + producer signature; push via the authenticated guardian client; return the proposal. The artifact is **not stored**.
- `prepare_custom_execution` MUST: reject a non-custom proposal; if not ready → not-ready error with **no side effects** (FR-008/FR-023); deserialize artifact (else error); derive the summary and assert `commitment == proposal_id` (else binding-mismatch error, **before** the ack request, FR-007/FR-020/FR-023); fetch the GUARDIAN ack; return the advice. It MUST NOT submit.
...
| Artifact undeserializable | `InvalidConfig` ("failed to decode transaction artifact") | thrown error | none |

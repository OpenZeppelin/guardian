...
4. **Given** a custom proposal exported and re-imported (offline flow), **When** a producer calls prepare-execution **and** supplies the executable artifact, **Then** it returns advice as in the online case; **and when** prepare-execution is attempted **without** the artifact, **Then** it is refused with a clear error and no side effects.
...
- **Rebuilt artifact (fresh salt) at execution**: if the artifact the producer re-supplies to the prepare step differs from the one signed (e.g. rebuilt with a fresh salt) → different summary commitment → rejected by the FR-007 binding check before any acknowledgment request or submission. The SDK does not store the artifact (FR-015); the producer reproduces it from its own recipe, so it must preserve the salt and all inputs.
...
- **Execute called on a custom proposal without the artifact** (e.g. an imported proposal): rejected with a clear error explaining the producer must supply the executable artifact (see FR-022); no acknowledgment request, no submission.
...
- **FR-015**: The serialized executable artifact MUST NOT be stored anywhere by the SDK or server — not embedded in proposal metadata, not persisted server-side. The integration owns its transaction recipe and supplies the artifact at create (to derive the summary) and at execute-prep (for the binding check). This keeps the server a minimal coordinator (FR-010) and means custom execution is performed by a party holding the recipe.
- **FR-016**: If the supplied artifact cannot be deserialized into a valid transaction request, the SDK MUST fail with a clear error and MUST NOT register (at create) or submit (at execute) anything.
- **FR-017**: Determinism across create→execute MUST be conveyed by the executable artifact itself: the replay-protection salt and all other inputs are part of it, so re-supplying it reproduces the signed summary commitment. The SDK MUST NOT generate or inject a salt for a producer-supplied artifact — this differs from built-in types by necessity (built-in types let the SDK generate the salt, persist it in proposal metadata, and rebuild at execution; custom types have no SDK recipe to rebuild, so the artifact is the durable source of determinism).
...
- **FR-022** *(export/import and offline)*: Exported and imported custom proposals MUST support review and signing identically to built-in types (label and summary round-trip; already delivered under #266). Executing a custom proposal — online or offline — additionally requires the producer-supplied executable artifact, which is NOT contained in the proposal or its exported form; the SDK MUST surface a clear error if asked to execute a custom proposal without the artifact. Bundling the artifact into the exported proposal is out of scope for v1 (the producer carries it separately).
- **FR-023** *(no side effects on failed execution)*: On a binding mismatch (FR-007), a not-ready proposal (FR-008), or a missing/undeserializable artifact (FR-016/FR-022), execution MUST NOT request a Guardian acknowledgment and MUST NOT submit anything on-chain. This MUST be covered by explicit tests.
...
- **SC-007**: In every failed-execution path (binding mismatch, not-ready, missing/undeserializable artifact), no Guardian acknowledgment is requested and nothing is submitted on-chain — verified by explicit tests.
...
- The serialized transaction is not stored by the SDK or server (FR-015); the integration owns its recipe and supplies the artifact at create and execute-prep. Consequently custom execution is performed by a party holding the recipe (typically the producer), not an arbitrary cosigner.

...
### ExecutableArtifact *(new input; central contract — FR-019)*
...
- **Lifecycle**: produced by the integration → supplied at `propose_custom` (SDK derives the summary, does not store the artifact) → the integration keeps its own recipe → re-supplied at `prepare_custom_execution` for the binding check. Not stored by the SDK or server (FR-015).
...
| `tx_summary` | summary | Derived by the SDK from the artifact; what the server validates and what is signed. |
...
- **Rule**: `commitment(execute_for_summary(deserialize(artifact)))` **==** `proposal.id`.
...
```
ExecutableArtifact ──derive──▶ TransactionSummary ──commitment──▶ Proposal.id
   (integration-owned)                                                ▲
...
- `propose_custom` with built-in label, undeserializable artifact, or non-executable/invalid-for-state tx → **rejected**, nothing registered.
- `prepare_custom_execution` on a non-custom proposal, not-ready, undeserializable artifact, or binding mismatch → **rejected**, no ack request.

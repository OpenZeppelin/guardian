...
- Q: How granular should the action-kind enumeration on the wire be? → A: Hybrid shape: a stable closed `category` enum plus an optional `kind` string. `category` ∈ {`asset_transfer`, `asset_swap`, `note_consumption`, `note_creation`, `account_storage_change`, `guardian_switch`, `custom`}. `kind` echoes `metadata.proposal_type` (e.g. `add_signer`, `remove_signer`, `change_threshold`, `update_procedure_threshold`, `p2id`, `consume_notes`, `switch_guardian`) when the delta carries one; null for single-key push deltas and any delta without `proposal_type` metadata. `category` is signing-scheme-agnostic; multisig admin operations land in `account_storage_change` with their specific operation surfaced via `kind`.
...
- **Single-key push deltas and EVM deltas have no `proposal_type` metadata.** `category` MUST still be derived from the on-chain `TransactionSummary` (note kinds + account-state delta) and MUST never be null; `kind` is null in this case and the dashboard renders from `category` alone. If no inference rule matches, `category` falls back to `custom`.
- **Delta payload metadata is free-form JSON.** A malformed or unexpected shape (e.g., schema drift, missing required sub-field) must not break the listing or detail endpoint for the rest of the page; the failing field is absent and the rest of the entry is returned.
- **Listing pagination interacts with enrichment cost.** Deriving action kind and asset/note summaries from every payload must not regress listing latency past current dashboard expectations even on accounts with high transaction volume.
...
- **FR-002**: Each listing entry MUST include two action-classification fields: a `category` and an optional `kind`. `category` is a closed, stable enumeration with the values `asset_transfer`, `asset_swap`, `note_consumption`, `note_creation`, `account_storage_change`, `guardian_switch`, and `custom`; it MUST be present and non-null on every entry. `kind` is a string that echoes the underlying `metadata.proposal_type` (e.g. `add_signer`, `remove_signer`, `change_threshold`, `update_procedure_threshold`, `p2id`, `consume_notes`, `switch_guardian`) when the delta carries one, and is null otherwise (including for all single-key push deltas). The `category` enumeration MUST be documented as part of the wire contract; adding a new value to it is a contract change. `kind` is intentionally not a closed enumeration so that adding a new `proposal_type` does not force a wire-contract change.
- **FR-002a**: The mapping from `metadata.proposal_type` (when present) to `category` MUST be deterministic and documented. At minimum: `p2id` → `asset_transfer`; `consume_notes` → `note_consumption`; `switch_guardian` → `guardian_switch`; `add_signer`, `remove_signer`, `change_threshold`, `update_procedure_threshold` → `account_storage_change`.
- **FR-002b**: When `metadata.proposal_type` is absent (single-key push deltas, EVM deltas, or any delta whose payload carries no Guardian-side proposal metadata), `category` MUST be inferred from the on-chain `TransactionSummary` using the input/output note kinds (Miden standard notes: `p2id`, `p2ide`, `pswap`, `mint`, `burn`) and the account-state delta. If no rule matches with confidence, `category` MUST fall back to `custom` rather than guess.
- **FR-003**: Each listing entry MUST include, when applicable to the action kind, summary fields for the assets involved (asset identifier and amount), the recipients/counterparties involved, and the counts of input notes consumed and output notes created.
- **FR-004**: When a summary field cannot be safely extracted from the underlying payload (malformed or missing sub-field), the listing entry MUST still be returned with all extractable fields populated and the unavailable fields absent or explicitly null, rather than the entry being dropped or the endpoint failing.
...
- **FR-012**: Each decoded note (input or output) MUST include, when applicable, the asset(s) and amount(s) carried, the sender/recipient identifier, and a stable identifier for the note itself. The note script MAY be returned but MUST be under a clearly-marked optional field that callers can omit reading without losing the primary semantics.
...
- **FR-014**: Storage/account-field changes MUST be expressed as a structured list of changed fields/slots with their before/after values, not as a free-form blob.
- **FR-015**: The raw underlying transaction summary MAY be returned, but only under a clearly-marked optional/debug field that is not part of the primary dashboard contract.
...
- **Action category**: A closed, stable enumeration on every listing/detail entry describing what a delta did at the coarsest useful level: `asset_transfer`, `asset_swap`, `note_consumption`, `note_creation`, `account_storage_change`, `guardian_switch`, `custom`. Signing-scheme-agnostic — single-key push and multisig deltas use the same values.
- **Action kind**: An optional, open-ended string echoing `metadata.proposal_type` when the delta carries one (e.g. `p2id`, `consume_notes`, `add_signer`, `remove_signer`, `change_threshold`, `update_procedure_threshold`, `switch_guardian`). Null for single-key push deltas and any delta whose payload has no Guardian-side proposal metadata. Lets the dashboard render precise labels (e.g. "Removed signer X") without forcing the closed `category` enum to grow.
- **Activity summary**: The per-entry derived fields shown in the listing — action kind, assets/amounts, recipients/counterparties, input/output note counts. Each is optional and absent when the underlying payload does not carry it.
- **Decoded note**: A structured projection of an input or output note carried by a transaction, exposing asset/amount, sender/recipient, and a stable note identifier. The note script is treated as optional debug content.
...
- **Storage/account change**: A structured projection of a single changed account-level field or storage slot, expressing the before/after value.
...
- **SC-002**: Every canonical delta returned in the listing endpoints carries a non-null `category` value; `custom` is acceptable but null/missing is not. `kind` may be null (and is null for single-key push deltas by design).

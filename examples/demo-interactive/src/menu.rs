...
use crate::display::{print_menu_header, print_menu_option, print_error};
...
pub fn print_menu(state: &SessionState) {
    ...
    print_menu_option("2", "Create multisig account", state.has_keypair() && !state.has_account());
    print_menu_option("3", "Configure account in PSM", state.has_account() && state.is_psm_connected());
    print_menu_option("4", "Pull account from PSM", !state.has_account() && state.is_psm_connected());
    print_menu_option("5", "Pull deltas from PSM", state.has_account() && state.is_psm_connected());
    print_menu_option("6", "Add cosigner (update to N+1)", state.has_account());
    print_menu_option("7", "Sign pending transaction", state.has_account() && state.pending_tx_store.has_pending());
    print_menu_option("8", "Finalize pending transaction", state.has_account() && state.pending_tx_store.has_pending());
...
pub fn parse_menu_choice(choice: &str, state: &SessionState) -> Option<MenuAction> {
    match choice {
        ...
        "5" if state.has_account() && state.is_psm_connected() => Some(MenuAction::PullDeltasFromPsm),
        "6" if state.has_account() => Some(MenuAction::AddCosigner),
        "7" if state.has_account() && state.pending_tx_store.has_pending() => Some(MenuAction::SignTransaction),
        "8" if state.has_account() && state.pending_tx_store.has_pending() => Some(MenuAction::FinalizePendingTransaction),

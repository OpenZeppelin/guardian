...
Fetches proposals from the GUARDIAN server and reconciles local state. A proposal GUARDIAN reported on an earlier sync but no longer reports (executed, canonicalized, or abandoned) is pruned from the cache. A proposal GUARDIAN has never reported to this client (freshly created, or imported and not yet synced) survives the first listing that omits it, absorbing a read-your-writes lag, and is pruned once a second consecutive listing omits it. The response is verified in full before the cache changes, and overlapping calls share a single in-flight sync:

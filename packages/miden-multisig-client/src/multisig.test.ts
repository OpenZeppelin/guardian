...
describe('Multisig', () => {
  ...
  describe('syncProposals', () => {
    ...
    it('should reject proposals whose metadata does not match tx_summary', async () => {
      ...
      expect(listed.verificationError).toContain(
    ...
    it('lists a verified and an unverifiable proposal side by side (issue #462)', async () => {
      ...
      expect(bad?.verificationError).toContain('metadata does not match tx_summary');
      expect(good?.verificationError).toBeUndefined();
  ...
  describe('executeProposal', () => {
    ...
    it('refuses to sync a proposal GUARDIAN served without a salt', async () => {
      ...
      const [listed] = await multisig.syncProposals();
      expect(listed.verificationError).toContain('has no salt');
    });

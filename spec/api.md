...
| client | `GET /transactions` | signed headers | Paginated canonical transaction history with decoded note summaries |
...
- **Pagination.** Paginated endpoints (the dashboard feeds and the
  client `GET /transactions`) return `{ items, next_cursor }`; `limit`
...
- **`GET /transactions`.** Canonical deltas only, newest-first by nonce, with
...
- `GetTransactionHistory(GetTransactionHistoryRequest) -> GetTransactionHistoryResponse`

...
| client | `GET /history` | signed headers | Paginated canonical transaction history with decoded note summaries |
...
- **Pagination.** Paginated endpoints (the dashboard feeds and the
  client `GET /history`) return `{ items, next_cursor }`; `limit`
...
- **`GET /history`.** Canonical deltas only, newest-first by nonce, with

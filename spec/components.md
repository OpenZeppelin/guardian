...
- A process-global metrics recorder collects counters, gauges, and histograms from instrumentation across the server: an HTTP middleware on the API router (request rate/latency/in-flight with route-template labels), a tower layer on the gRPC server (per-method counts, latency, and in-flight depth with the status code read from HTTP/2 trailers), a transparent decorator around the storage backend (per-operation latency and outcomes), outbound Miden chain-node RPC counters/latency, DB connection-pool saturation gauges (postgres builds), and lifecycle counters in the canonicalization worker, proposal services, account creation, operator auth handlers, and the rate limiter.
